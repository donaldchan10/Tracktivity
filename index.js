...
app.get('/all', isAuthenticated, async function(request, response, next) {
  ...
  let results = await dbo.get_all(req.user.userId)
...
app.get('/name', isAuthenticated, function(req,res){
  console.log("profile:", req.user.firstName);
  res.send(req.user);
});
...
//put your userid
dbo.testDB(109082754974002320000).catch(
  function (error) {
...
function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    //call db Profile store function
    storeProf(userid, firstName);
...
//Was passport.deserializeUser((userid, done) => 
passport.deserializeUser(async (userid, done) => {
    ...
    let userData = await getProf(userid);
    done(null, userData);
});

app.use(expressSession({
  secret:'coders are cracked',
  maxAge: 6*60*60*1000,
  name:'fitness-cookie-session'}));
...
async function storeProf(userId, firstName){
  await dbo.storeProfile(userId, firstName);
}

async function getProf(userId){
  let result = await dbo.getProfile(userId);
  return result;
}
